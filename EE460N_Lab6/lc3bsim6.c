...
int pcmux = 0, v_mem_ld_cc = 0, v_mem_ld_reg = 0, mem_reg_id;
...
void MEM_stage() {
    ...
    int ii,jj = 0;
    /*SR.V Logic*/
    pcmux = 0, v_mem_ld_cc = 0, v_mem_ld_reg = 0, mem_stall = 0;
    
    /* your code for MEM stage goes here */
    /*dcache*/
    int read_word, write_word = 0, ready = 0, mem_w0 = 1, 
        mem_w1 = 0, address = 0, data = 0;

    address = Low16bits(PS.MEM_ADDRESS);
    int cacheEn = 0;
    ...
    if(Get_DCACHE_RW(PS.MEM_CS)) {
        if(Get_DATA_SIZE(PS.MEM_CS) == 1) {
            ...
            /*high bits*/
            write_word = Low16bits((address >> 8) & 0x00FF);
            mem_w1 = 1;

        } else {
            /*low bits*/
            write_word = Low16bits(address & 0x00FF);
            mem_w0 = 1;
        }
    }

    if(cacheEn == 1) { 
        dcache_access(address, &read_word, write_word, &ready, mem_w0, mem_w1);
        if(ready == 1) {
            mem_stall = 0;
            if(Get_DATA_SIZE(PS.MEM_CS) == 1) {
                /*word*/
                data = Low16bits(read_word);
            } else { /*sign extend?*/
                /*byte*/
                if((address & 0x01) == 1) {
                    data = Low16bits(read_word << 8);
                } else {
                    data = Low16bits(read_word);
                }
            }
    ...
    if(PS.MEM_V == 1) {
        if(Get_TRAP_OP(PS.MEM_CS) == 1) {
            ...
            int brcc = (PS.MEM_IR >> 9) & 0x07;
            if(brcc == PS.MEM_CC && brcc != 0) {
                pcmux = 1;
            }
        }
    }
...
int v_agex_ld_cc = 0, v_agex_ld_reg = 0, agex_reg_id;
/************************* AGEX_stage() *************************/
void AGEX_stage() {
    ...
    if(!mem_stall) {
        LD_MEM = 1;
    } else if(mem_stall || v_mem_br_stall) {
        NEW_PS.MEM_V = 1;
    } else {
        NEW_PS.MEM_V = 0;
    }
    ...
    switch(Get_ADDR2MUX(PS.AGEX_CS)) {
        ...
        case 1: {
            /*[5:0]*/
            int sext = Low16bits((PS.AGEX_IR >> 5) & 0x0001);
            if(sext == 1) {
                addr2mux = Low16bits((PS.AGEX_IR & 0x03F) | 0xFFC0);
            } else {
                addr2mux = Low16bits(PS.AGEX_IR & 0x003F);
            }
        } break;
        case 2: {
            /*[8:0]*/
            int sext = Low16bits((PS.AGEX_IR >> 8) & 0x0001);
            if(sext == 1) {
                addr2mux = Low16bits((PS.AGEX_IR & 0x01FF) | 0xFE00);
            } else {
                addr2mux = Low16bits(PS.AGEX_IR & 0x01FF);
            }
        } break;
        case 3: {
            /*[10:0]*/
            int sext = Low16bits((PS.AGEX_IR >> 10) & 0x0001);
            if(sext == 1) {
                addr2mux = Low16bits((PS.AGEX_IR & 0x07FF) | 0xF800);
            } else {
                addr2mux = Low16bits(PS.AGEX_IR & 0x07FF);
            }
        }
    }
    ...
    if(Get_ADDRESSMUX(PS.AGEX_CS) == 1) {
        address = Low16bits(addr1mux + addr2mux);
    } else {
        address = Low16bits((0x00FF & PS.AGEX_IR) << 1);
    }

    /*SHF*/
    int shfV;
    int shiftInput = (PS.AGEX_IR >> 4) & 0x0003, shA = (PS.AGEX_IR) & 0x000F;
    switch(shiftInput) {
        case 0: {
            /* left shift */
            shfV = Low16bits(PS.AGEX_SR1 << shA);
        } break;
        case 1: {
            /* right shift logical */
            shfV = Low16bits(PS.AGEX_SR1 >> shA);
        ...
        case 2: {
            /*rshfa*/
            int a, temp = PS.AGEX_SR1;
            for(a = 0; a < shA; a++) {
                temp = Low16bits((PS.AGEX_SR1 & 0x8000) | (temp >> 1));
            }
            shfV = Low16bits(temp);
        }
    }
    ...
    if(Get_SR2MUX(PS.AGEX_CS) == 1) {
        /*immediate*/
        int sext = (PS.AGEX_IR >> 4) & 0x01;
        if(sext == 1) {
            operand = Low16bits((PS.AGEX_IR & 0x001F) | 0xFFE0);
        } else {
            operand = Low16bits(PS.AGEX_IR & 0x001F);
        }
    } else {
        operand = Low16bits(PS.AGEX_SR2);
    }

    /*ALU*/
    int sr1 = PS.AGEX_SR1, aluV;
    int aluS = Get_ALUK(PS.AGEX_CS);
    
    switch(aluS) {
        case 0: {
            /*add*/
            aluV = Low16bits(sr1 + operand);
        } break;
        case 1: {
            /*and*/
            aluV = Low16bits(sr1 & operand);
...
void DE_stage() {
    ...
    int* mcinst = CONTROL_STORE[CONTROL_STORE_ADDRESS];
    v_de_br_stall = Get_DE_BR_STALL(mcinst) && PS.DE_V;
    ...
    /*REG*/
    int sr2;
    if(((PS.DE_IR >> 13) & 0x0001) == 1) {
        sr2 = (PS.DE_IR >> 9) & 0x0007;

    } else {
        sr2 = (PS.DE_IR) & 0x0007;
    }
    int sr1 = (PS.DE_IR >> 6) & 0x0007;
    int sr1V = Low16bits(REGS[sr1]);
    int sr2V = Low16bits(REGS[sr2]);

    int dr;
    if(Get_DRMUX(mcinst) == 1) {
        dr = 7;
    } else {
        dr = (PS.DE_IR >> 9) & 0x0007;
    }

    /*DCL*/
    dep_stall = 0;
    if(PS.DE_V == 1) {
        if(Get_SR1_NEEDED(mcinst)) {
            if((sr1 == agex_reg_id && v_agex_ld_reg == 1) || 
                (sr1 == mem_reg_id && v_mem_ld_reg == 1) || 
                (sr1 == sr_reg_id && v_sr_ld_reg == 1)) {
                dep_stall = 1;
            }
        }
        if(Get_SR2_NEEDED(mcinst)) {
            if((sr2 == agex_reg_id && v_agex_ld_reg == 1) || 
                (sr2 == mem_reg_id && v_mem_ld_reg == 1) || 
                (sr2 == sr_reg_id && v_sr_ld_reg == 1)) {
                dep_stall = 1;
            }
        }
        if(Get_DE_BR_OP(mcinst)) {
    ...
    if (LD_AGEX) {
        ...
        NEW_PS.AGEX_SR1 = Low16bits(sr1V);
        NEW_PS.AGEX_SR2 = Low16bits(sr2V);
        NEW_PS.AGEX_CC = (N << 2) | (Z << 1) | P;
        NEW_PS.AGEX_DRID = dr;
        if(PS.DE_V && !mem_stall && !dep_stall) {
            NEW_PS.AGEX_V = 1;
        } else NEW_PS.AGEX_V = 0;
...
void FETCH_stage() {
    ...
    /*ldpc*/
    int ld_pc = 0;
    if(!mem_stall && !dep_stall && !v_de_br_stall && !v_agex_br_stall && ld_de) {
        ld_pc = 1;
    } else if((mem_stall || dep_stall)) {
        NEW_PS.DE_V = 1;
    } else {
