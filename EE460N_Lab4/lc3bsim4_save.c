...
void eval_micro_sequencer() {
   ...
   */
    ...
    if(GetIRD(minst) != 1 || flag) {
        /*evaluate j bits*/
        /*check cond0 and cond1*/
        /*check branch, ready, addressmode*/
        int condL = GetCOND(minst) & 0x03;
        int condH = (GetCOND(minst) & 0x0C) >> 2;
    ...
    if(CURRENT_LATCHES.EXC == 0) {
        ...
        */
        if(GetLD_MAR(mcinst) == 1) {
            ...
            if(BUS%2 == 1 && GetDATA_SIZE(mcinst) == 1) {
                /*unaligned error*/
        ...
        */
        if(GetLD_PSR(mcinst) == 1) {
            if(GetPSRC(mcinst) == 1) {
                ...
                NEXT_LATCHES.P = (BUS & 0x02) >> 1;
                NEXT_LATCHES.Z = BUS & 0x01;
